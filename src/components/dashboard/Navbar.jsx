...
const Navbar = () => {
  ...
  const navItems = [
    ...
    // { id: 'PROFILE', label: 'Profile', path: '/dashboard/profile', icon: <User size={16} /> }
  ];
  ...
  return (
    <nav className="border-b border-zinc-900 bg-[#000000] sticky top-0 z-40 px-4 md:px-8 py-3.5 flex items-center justify-between select-none">
                ...
                {!connected ? (
                  ...
                  <div className="flex items-center gap-2 bg-zinc-900/50 border border-zinc-800 rounded-xl p-1 pr-3">
                    ...
                    <button
                      ...
                      className="p-1 rounded-lg text-white hover:bg-zinc-800 hover:text-red-400 transition-all cursor-pointer"
                    >
                      <LogOut size={14} />
                    </button>
                  </div>
      ...
      {/* Mobile Nav */}
      <div className="md:hidden fixed bottom-4 left-1/2 -translate-x-1/2 bg-black/90 backdrop-blur-lg border border-zinc-800 rounded-2xl px-4 py-3 flex items-center gap-4 shadow-2xl z-50 overflow-x-auto">
        {navItems.map((item) => {
          const isActive = location.pathname === item.path;
          ...
          return (
            <Link
              key={item.id}
              to={item.path}
              className={`flex flex-col items-center gap-0.5 min-w-[44px] transition-all ${isActive ? 'text-amber-500' : 'text-white hover:text-amber-500'
                }`}
            >
              <span className="text-[16px]">{item.icon}</span>
              <span className={`text-[8px] font-bold font-display tracking-wider ${isActive ? 'text-amber-500' : 'text-white'
                }`}>
